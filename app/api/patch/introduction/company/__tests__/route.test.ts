...
vi.mock('~/app/api/company/cache', () => ({
  invalidateCompanyListCache: invalidateListCacheMock
}))
...
import { POST, PUT } from '~/app/api/patch/introduction/company/route'

const INPUT = { patchId: 42, companyId: [7, 8] }
...
// 四个关联变更 handler(company/tag 各 POST+PUT)共用同一套编排契约,
// 抽 helper 后任何一处漂移都会在这里变红
const describeOrchestrationContract = (
  ...
  describe(`${label} 公共编排`, () => {
    it('解析失败直接回错误串, 不查补丁', async () => {
      parseMock.mockResolvedValue('会社 ID 必须为数字')
      const res = await handler(request)
      expect(await res.json()).toBe('会社 ID 必须为数字')
...
describe('会社关联事务体', () => {
  it('POST 只新增尚未关联的会社并事务内入队', async () => {
    relationFindManyMock.mockResolvedValue([{ company_id: 7 }])
    await POST(request)
    expect(relationCreateManyMock).toHaveBeenCalledWith({
      data: [{ patch_id: 42, company_id: 8 }]
    })
  ...
  it('PUT 只删除确实存在的关联并事务内入队', async () => {
    relationFindManyMock.mockResolvedValue([{ company_id: 8 }])
    await PUT(request)
    expect(relationDeleteManyMock).toHaveBeenCalledWith({
      where: { patch_id: 42, company_id: { in: [8] } }
    })
